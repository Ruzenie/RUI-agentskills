...
function isCompactBundle(bundleSize) {
  return (bundleSize.includes('KB') && !bundleSize.includes('MB')) || bundleSize.includes('按需');
}
...
function markdownRecommend(results) {
  const lines = [];
  lines.push('# UI组件库推荐结果');
  lines.push('');
  results.forEach((item, idx) => {
    const lib = item.library;
    lines.push(`## ${idx + 1}. ${lib.name} (${item.score.toFixed(1)}分)`);
    lines.push(`- 类型: ${lib.category}`);
    lines.push(`- 框架: ${lib.framework.join(', ')}`);
    lines.push(`- Stars: ${lib.githubStars.toLocaleString()}`);
    lines.push(`- 包体积: ${lib.bundleSize}`);
    if (item.reasons.length > 0) {
      lines.push(`- 匹配理由: ${item.reasons.join(' / ')}`);
    }
    lines.push('');
  });
  return lines.join('\n');
...
function markdownEvaluate(result, dimensions) {
  const lines = [];
  lines.push('# UI组件库评估结果');
  lines.push('');
  lines.push(`- 维度: ${dimensions.map((d) => d.id).join(', ')}`);
  lines.push('');
...
function main() {
  ...
  if (command === 'recommend') {
    ...
    if (!prefs.framework || !prefs.projectType) {
      throw new Error('recommend 需要 --framework 和 --project-type');
    }
  ...
  if (command === 'evaluate') {
    ...
    if (libraryIds.length === 0) {
      throw new Error('evaluate 需要 --libraries');
    }
  ...
  if (command === 'compare') {
    const libraryIds = asList(options.libraries);
    if (libraryIds.length === 0) {
      throw new Error('compare 需要 --libraries');
    }
